...
def parse_args():
    ...
    parser.add_argument("--video_name", type=str, default="16536366")
...
def main(cfg):
    ...
    video_list = os.listdir(testset_dir)
    a=range(0,50)
    for i in a:
        test_set = TestsetLoader(testset_dir+ video_list[i] +'/', upscale_factor)
        test_loader = DataLoader(test_set, num_workers=1, batch_size=1, shuffle=False)
        net = SOFVSR(upscale_factor=upscale_factor)
        ckpt = torch.load('./log/BI_x4_iter15100.pth')#176700  101000
        ...
        for idx_iter, (LR_y_cube, SR_cb, SR_cr) in enumerate(test_loader):
            ...
            if not os.path.exists('results/' + video_list[i]):
                os.mkdir('results/' + video_list[i])
            SR_rgb.save('results/'+video_list[i]+'/sr_'+ str(idx_iter+1).rjust(4,'0') + '.png')#str(idx_iter+2)
